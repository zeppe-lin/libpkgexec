...
flags=$(PKG_CONFIG_PATH="$consumer_pc" pkg-config --cflags libpkgexec)
if [ "$link_mode" = static ]; then
  libs=$(PKG_CONFIG_PATH="$consumer_pc" pkg-config --static --libs libpkgexec)
else
  libs=$(PKG_CONFIG_PATH="$consumer_pc" pkg-config --libs libpkgexec)
fi
sanitize=
