...
[ "$(sed -n 's/^Version:[[:space:]]*//p' "$metadata")" = 2.2.0 ] || fail 'wrong module version'
...
libpkgsource < 5.0.0'
for requirement in 'libpkgsource >= 4.0.0' 'libpkgsource < 5.0.0'; do
...
[ "$(printf '%s\n' "$requires" | LC_ALL=C sort)" = "$(printf '%s\n' "$expected" | LC_ALL=C sort)" ] ||
  fail 'public requirements are not the exact source-4 interval'
