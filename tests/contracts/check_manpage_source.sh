...
for page in libpkgexec.3 pkgexec_backend.3 pkgexec_control.3 pkgexec_profile_codec.3 pkgexec_request.3 pkgexec_result.3 pkgexec_result_codec.3 pkgexec_semantics.7; do
  ...
  printf '%s\n' "$first" | grep -F '| Version 2.2.0' >/dev/null || fail "wrong manual version title: $page"
