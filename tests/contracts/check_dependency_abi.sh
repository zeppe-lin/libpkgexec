...
for expected in libpkgsource.so.4 libcrypto.so.3; do
  printf '%s\n' "$needed" | grep -Fx "$expected" >/dev/null ||
    fail "missing direct NEEDED $expected"
done
for obsolete in libpkgsource.so.1 libpkgsource.so.2 libpkgsource.so.3; do
  ! printf '%s\n' "$needed" | grep -Fx "$obsolete" >/dev/null ||
    fail "obsolete source provider generation admitted: $obsolete"
done
